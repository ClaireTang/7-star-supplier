...
const post = (url, data, callback,complete) => {
	...
	if (urlList.indexOf(url) >= 0) {
		...
		if (!userToken) {
			...
			// 设置header头 Authorizetion
			let auth = { 'Authorization' : `Bearer ${userToken}`}
			// let auth = { 'Authorization' : 'Bearer 6f4922f45568161a8cdf4ad2299f6d2381f859a96d25dec38bf38af18660f4611e01c320'}
			Object.assign(header,auth)
		}
	}
	uni.request({
		...
		success: (response) => {
			...
			if ( result.state < 0) {
				// 登录信息过期或者未登录:总之token验证失败
				if (result.state == '-10010') {
					db.del("userToken");
					uni.showToast({
						title: result.msg,
						icon: 'none',
	...
	uni.chooseImage({
		...
		success: (res) => {
			...
			for (var i = 0; i < tempFilePaths.length; i++) {
				uni.uploadFile({
					url: apiBaseUrl + 'merapi/commercial/getImage',
...
// 用户登录
export const login = (data, callback) => post('login/login', data, callback);
